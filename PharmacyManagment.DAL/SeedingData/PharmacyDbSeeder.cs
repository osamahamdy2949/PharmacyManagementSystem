...
public static class PharmacyDbSeeder
{
    ...
    public static async Task SeedAsync(PharmacyDbContext context)
    {
        ...
        context.PurchaseInvoiceItems.AddRange(
            new PurchaseInvoiceItem { PurchaseInvoiceId = purchaseInvoice.Id, MedicineId = medicines[0].Id, MedicineBatchId = batchForMed0.Id, Quantity = 50, UnitPrice = 5.50m, BatchNumber = "BATCH-001B", ExpiryDate = DateTime.Today.AddMonths(18), SellingPrice = 8.00m },
            new PurchaseInvoiceItem { PurchaseInvoiceId = purchaseInvoice.Id, MedicineId = medicines[2].Id, MedicineBatchId = batchForMed2.Id, Quantity = 30, UnitPrice = 3.00m, BatchNumber = "BATCH-003B", ExpiryDate = DateTime.Today.AddMonths(12), SellingPrice = 6.00m }
        );
        ...
        context.SalesInvoiceItems.AddRange(
            new SalesInvoiceItem { SalesInvoiceId = salesInvoice.Id, MedicineId = medicines[1].Id, MedicineBatchId = batches[1].Id, Quantity = 2, UnitPrice = 4.50m },
            new SalesInvoiceItem { SalesInvoiceId = salesInvoice.Id, MedicineId = medicines[3].Id, MedicineBatchId = batches[3].Id, Quantity = 5, UnitPrice = 7.50m }
        );
